...
module.exports = async(req,res)=>{
    ...
    //TRAE TODOS LOS DOCUMENTOS HISTORICOS EN DONDE EN ALGUN MOVIMIENTO EL USER_DESTNO ESTUVIESE INVOLUCRADO.
    try{
        const [result] = await pool.query(`SELECT d.id, d.datetime_creacion, d.nota, d.pertinente AS idPertinente, a.descripcion AS pertinente, d.asunto, m.id_estado, e.descripcion AS estado, m.id AS ordenmov, m.datetime_recibido, m.user_destino, u.nombre, d.tipodoc, d.origen, d.fojas
            FROM documento AS d 
            LEFT JOIN (SELECT m1.id_documento, MAX(m1.id) AS max_id FROM movimientos AS m1 WHERE m1.user_destino = ${user_destino} GROUP BY m1.id_documento) AS max_mov
            ...
            WHERE m.user_destino=${user_destino}
